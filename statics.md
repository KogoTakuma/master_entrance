...
二項分布とかポアソン分布の問題。標準的
### 設問3
2つの確率変数から、新しい確率変数をつくって共分散とか相関係数を求める問題。びびらず冷静に
<details> <summary>ポイント</summary>
  
 $`U = 5x -3, V = -3Y+2,E(UV)=E(-15XY+10X+9Y-6)`$ </details>
...
### 設問2
具体的な確率を求めて、検定を行う。
具体的な数値出すのは現実的でないのである程度ヒューリスティックに
<details> <summary>答案</summary>
...
<details> <summary>答案</summary>
 ...
 第一種の誤りと第二種の誤りはトレードオフ
 第一種の誤り(生産者のリスク)はcが大きくなるほど棄却されやすくなってしまうから、
 cが大きくなると第一種の誤りは大きくなる。
 第二種の誤りを小さくするには、cを大きくすればいい。
 よって$`c=3`$
 
 </details>
 
### 設問4 要検討 意見募集
帰無仮説を$`H_0 p=1/3`$として推定を行う。
僕はいいやり方が思い浮かばなかったら$`k=0 \sim 8`$に対して全て求めて、小さくなるものから選ぶ
対象にはならい点に注意

<details> <summary>答案</summary>
  
 $`P(k=8) < P(k=7) < P(k=6) < P(k=0) < P(k=5)`$より
...
[モンモール問題の解説](https://www.ms.u-tokyo.ac.jp/~aida/paper/preprint/montmort.pdf)
#### (1)
簡単な確率の問題
#### (2)
簡単な確率の問題
